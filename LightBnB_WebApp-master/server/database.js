...
// the following assumes that you named your connection variable `pool`, use to confirm connection to db
...
//when signing up a user object is passed into addUser, then ln83, the values are object user based
const addUser = (user) => {
  return pool
...
const getAllProperties = (options, limit = 10) => {
  //set queryParams empty array for user query input
  const queryParams = [];
  ...
  if (options.minimum_rating) {
    ...
    queryString += ` HAVING avg($${queryParams.length})`;
  }
